...
public class CustomSkinTexture {
    public static final ResourceLocation LOCATION =
            ResourceLocation.fromNamespaceAndPath(RumbleFruitMod.MOD_ID, "player_skin_custom");
    ...
    public static void ensureRegistered() {
        ...
        try (InputStream in = mc.getResourceManager().open(
                ResourceLocation.fromNamespaceAndPath(RumbleFruitMod.MOD_ID, "textures/player_skin.png"))) {
            mc.getTextureManager().register(LOCATION, new DynamicTexture(NativeImage.read(in)));
            ready = true;
        } catch (Exception e) {
            System.out.println("[rumblefruit] failed to load bundled skin: " + e.getMessage());
        }
    }
