...
public class ClientRpgCamera {
    ...
    public static void applyCinematic(net.minecraft.client.Camera camera, net.minecraft.world.entity.Entity entity) {
        ...
        com.lightningfruit.mixin.CameraAccessor acc = (com.lightningfruit.mixin.CameraAccessor) camera;
        acc.lightningfruit$setPosition(cinePos);
        acc.lightningfruit$setDetached(true);
        // frame the caster
        double dx = eye.x - cinePos.x, dy = eye.y - 0.5 - cinePos.y, dz = eye.z - cinePos.z;
        double flat = Math.sqrt(dx * dx + dz * dz);
        acc.lightningfruit$setYRot((float) Math.toDegrees(Math.atan2(-dx, dz)));
        acc.lightningfruit$setXRot((float) Math.toDegrees(-Math.atan2(dy, flat)));
        acc.lightningfruit$setRoll(roll * 0.3F);
    }
